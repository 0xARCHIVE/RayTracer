...
class CameraSensor : public RayGenerator {
	protected:
		...
		float sensor_height;
	public:
		CameraSensor(const Vec3& _position, const Vec3& _angle, const Surface& _surface, int _resolution_x, int _resolution_y, float _sensor_width, float _sensor_height) : RayGenerator(_position, _angle);
